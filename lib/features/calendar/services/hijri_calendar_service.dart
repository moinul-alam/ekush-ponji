...
///
/// Remote offset correction:
///   Bangladesh determines Hijri dates by actual moon sighting, which can
///   differ from Umm al-Qura by ±1 day. HijriOffsetSyncService fetches
///   correction rules from GitHub on every launch. getHijriDate() applies
///   any matching offset after the table lookup.
class HijriCalendarService {
  ...
  ///
  /// After the table/algorithm lookup, applies any remote offset correction
  /// fetched by HijriOffsetSyncService. This allows correcting Bangladesh
  /// moon-sighting discrepancies without an app update.
  HijriDate getHijriDate(DateTime gDate) {
    final jd = _gregorianToJulian(gDate.year, gDate.month, gDate.day);
    final raw = _julianToHijri(jd);

    // Apply remote offset correction if any rule matches this date
  ...
  /// Shift a HijriDate by [offset] days, correctly rolling over
  /// month/year boundaries in both directions.
  HijriDate _applyOffset(HijriDate date, int offset) {
    if (offset == 0) return date;

    // Convert back to Julian Day, apply offset, convert back
    // This correctly handles month/year rollovers.
    final jd = _hijriToJulian(date);
    return _julianToHijri(jd + offset);
  }

  /// Approximate inverse: HijriDate → Julian Day.
  /// Used only for offset arithmetic, not for primary display.
  int _hijriToJulian(HijriDate h) {
    // Search the lookup table for the month start JD
    for (int i = 0; i < _monthStarts.length; i++) {
      final entry = _monthStarts[i];
      if (entry[1] == h.year && entry[2] == h.monthNumber) {
        return entry[0] + h.day - 1;
      }
    }
    // Fallback: tabular inverse
    final y = h.year;
    final m = h.monthNumber;
    final d = h.day;
    return d +
        (29 * m - 29 + (6 * m - 1) ~/ 11) +
        (10631 * y - 10617) ~/ 30 +
